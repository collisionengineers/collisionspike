...
-- -----------------------------------------------------------------------------
-- NEW Azure-era table (no Dataverse cr1bd_ ancestor). The single home for AI
...
COMMENT ON COLUMN ai_suggestion.embedding IS
  'rules-engine-v2 Phase 4 embedding prior (nearest-neighbours re-rank signal). Plain float8[] at current corpus scale (app-side cosine); pgvector is the documented, allowlisted-but-not-enabled scale path. DDL-only -- no writer/reader exists yet; populated only once the E2-governed live inbound_email export (labelled corpus) lands (docs/gated.md D6 #4).';
