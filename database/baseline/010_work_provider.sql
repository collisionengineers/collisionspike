-- =============================================================================
-- 010_work_provider.sql -- governed work-provider corpus
...
-- Conventions: id is the UUID primary key; created_at/updated_at mirror
-- the timestamps surfaced by the API.
-- =============================================================================
BEGIN;
...
CREATE TABLE work_provider (
  ...
  -- lowercase=EVA code, uppercase=archive/Case-PO prefix
  principal_code                  varchar(8),
  -- matching key, newline/JSON list; domain match only
  ...
  -- natural upsert key for the job-sheet reseed
  CONSTRAINT uq_work_provider_principal_code UNIQUE (principal_code)
);

COMMENT ON TABLE  work_provider IS 'Governed work-provider corpus; email-domain matched; principal_code seeds Case/PO.';
COMMENT ON COLUMN work_provider.known_email_domains IS 'Match key; domain only, no alias matching.';
