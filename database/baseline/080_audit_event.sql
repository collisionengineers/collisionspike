-- =============================================================================
-- 080_audit_event.sql -- append-only audit events
-- Domain-level workflow trail. case_id is NULLABLE (corpus-level events) and uses
-- ON DELETE SET NULL in 900 so history survives a removed
...
CREATE TABLE audit_event (
  ...
  name           varchar(400) NOT NULL,        -- one-line summary
...
COMMENT ON TABLE audit_event IS 'Append-only domain audit trail. RLS enforces no-update / admin-only-delete (see 900).';
