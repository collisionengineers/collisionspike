-- =============================================================================
-- 090_chaser.sql -- missing-item chasers (ADR-0003)
...
CREATE TABLE chaser (
  ...
  name              varchar(400) NOT NULL,       -- queue summary
  ...
  sent_by           varchar(200),
  sent_at           timestamptz,
  drafted_at        timestamptz,                 -- domain timestamp
  created_at        timestamptz NOT NULL DEFAULT now(),
  updated_at        timestamptz NOT NULL DEFAULT now()
);

COMMENT ON TABLE chaser IS 'Missing-items request; sending follows the current gate policy (ADR-0003).';
