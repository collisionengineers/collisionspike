#!/usr/bin/env bash
# =============================================================================
# provision.sh — provision the collisionspike Azure resources
# -----------------------------------------------------------------------------
# Creates the service resources in resource group rg-collisionspike-dev (UK South).
# Idempotent: every step
# guards on an `az ... show` so re-running converges instead of erroring.
#
# What it creates when separately authorized (nothing existing is deleted):
