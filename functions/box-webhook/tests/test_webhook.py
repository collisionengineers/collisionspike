...
disambiguation, and the full receiver order with a mocked DataverseClient.
...
# ==========================================================================
# 5. The receiver, end to end (mocked DataverseClient)
# ==========================================================================

class _FakeDataverse:
    """Stand-in DataverseClient: records calls, returns scripted answers."""
...
def _patch_dv(monkeypatch, fake):
    monkeypatch.setattr(function_app, "DataverseClient", lambda *a, **k: fake)
...
def test_receiver_rejects_replay_before_hmac(monkeypatch):
    fake = _FakeDataverse()
    _patch_dv(monkeypatch, fake)
    # 11 minutes old -> 400 replay, no Dataverse work.
...
def test_receiver_accepts_secondary_signature(monkeypatch):
    fake = _FakeDataverse(case_id="CASE-2")
    _patch_dv(monkeypatch, fake)
...
def test_receiver_transient_failure_then_box_retry_is_reprocessed(monkeypatch):
    ...
    class _FlakyDataverse(_FakeDataverse):
        ...
        def resolve_case_by_folder(self, folder_id):
            self.calls += 1
            if self.calls == 1:
                raise DataverseError("throttled", status=429)  # transient
            return self._case_id
...
def test_receiver_status_evaluate_failure_then_retry_advances(monkeypatch):
    ...
    class _StrandThenAdvance(_FakeDataverse):
        def __init__(self):
        ...
        def reinvoke_status_evaluate(self, case_id):
            ...
            if self._reinvoke_calls == 1:
                raise DataverseError("status-evaluate 503", status=503)  # genuine failure
            self.reinvoked.append(case_id)
            return True
...
def test_receiver_no_secret_or_key_in_response(monkeypatch):
    fake = _FakeDataverse(case_id="CASE-9")
    _patch_dv(monkeypatch, fake)
    resp = function_app.box_webhook(_signed_request(_UPLOAD_BODY))
