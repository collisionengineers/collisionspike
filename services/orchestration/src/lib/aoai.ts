 ...
 * the gated `triageClassify` activity (`functions/gated/triage-classify.ts`), which is
 ...
 * Per Microsoft's App-Service/Functions managed-identity REST reference, the legacy
 * `IDENTITY_ENDPOINT` token endpoint's `resource` query parameter takes the BARE Entra
...
/** `<scope>/.default` -> the bare Entra resource URI the legacy IDENTITY_ENDPOINT
