 ...
 * `promoteAcceptedSuggestion`) — mirrors the
...
import { callTriageModel } from '../../adapters/aoai.js';
...
interface TriageInput {
   ...
   *  re-resolving the provider here.
...
/**
 * Per-provider AI opt-out predicate. `work_provider.ai_allowed` is a
...
df.app.activity('triageClassify', {
  handler: async (input: TriageInput, ctx): Promise<TriageClassifyResult> => {
    ...
    // (services/data-api/src/features/assistant/suggestion-generation-routes.ts's
    ...
    // Per-provider AI opt-out (tracked in docs/operations/operator-actions.md). The orchestrator
