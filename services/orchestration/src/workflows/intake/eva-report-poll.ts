 ...
 * see docs/operations/operator-actions.md + the eva-sentry-api skill). This file is therefore the SMALLEST
 ...
 *   - the EVA client route itself (services/functions/eva-sentry/eva_client.py GetAvailableReports)
...
app.http('eva-report-poll-start', {
  ...
  handler: async (req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> => {
    if (!gates.evaApi()) {
      ctx.log('[eva-report-poll] skipped — EVA_API_ENABLED off (Minotaur single-principal limitation)');
...
df.app.activity('evaReportPollTick', {
  handler: async (_input: unknown, ctx): Promise<{ skipped?: string; marked?: number }> => {
    ...
    // GetAvailableReports route in services/functions/eva-sentry/eva_client.py lands with
