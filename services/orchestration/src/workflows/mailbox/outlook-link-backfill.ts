/**
 * Explicit, function-key protected missing-link repair. It is not timer/queue
...
export async function runOutlookLinkBackfill(
  ...
  for (const row of rows) {
    const lookup = await findStoredMessageLink(row.sourceMailbox, row.sourceMessageId);
    const payload = {
