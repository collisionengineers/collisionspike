...
describe('unavailable box-file-request-copy starter', () => {
  it('keeps the route without registering a Durable or Archive-copy worker', () => {
  ...
  it('returns an explicit 410 and the data-service route for valid work', async () => {
    const ctx = context();
    const response = await registrations.get('box-file-request-copy-start')!.handler(
      request({ caseId: CASE_ID, folderId: 'unused-folder-id' }),
      ctx,
    );
  ...
  it('rejects a missing or malformed case identifier without claiming success', async () => {
    const response = await registrations.get('box-file-request-copy-start')!.handler(
      request({ folderId: 'unused-folder-id' }),
      context(),
    );
