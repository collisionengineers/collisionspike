 ...
 * the data-service producer contract). The suggest-first lanes
 ...
 *      guard redirects only an inactive target to its verified survivor, and whose
...
import { attachmentBlobFileName, rawEmlFileName } from '../../platform/evidence-names.js';
...
import { isRetryableGraphError } from '../mailbox/outlook-move.js';

/** Keep in sync with the data-service producer contract. */
interface EvidenceBackfillJob {
  inboundEmailId: string;
  /** Optional because queued jobs may not carry a generation. */
  generation?: number;
