 ...
 *   freshly-subscribed mailbox from ingesting older messages.
 */
...
export async function createSubscription(
  ...
  return graphFetch<GraphSubscription>(SUBSCRIPTIONS_PATH, {
    ...
    // PATCH renewal cannot change an existing subscription's id type. Conversion belongs to the
...
export interface MaintenanceSummary {
  ...
  pruned: string[];
  /** Routine maintenance never rotates subscriptions. */
  rotated: string[];
  /** Subscriptions without immutable ids are renewed in place pending the approved cutover.
...
export async function runSubscriptionMaintenance(logger: MaintenanceLog): Promise<MaintenanceSummary> {
  ...
  // regardless of callback URL. An alternate-callback subscription counts as present here:
  ...
  for (const sub of subs) {
    ...
    // does not rotate alternate-id subscriptions: Graph forbids create-before-delete (409),
