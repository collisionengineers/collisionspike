...
# Module-level so they survive across warm-worker requests (the route layer builds a fresh
...
class BoxScopeError(BoxError):
    ...
    ``BOX_ALLOWED_ROOT_ID`` (the configured write-root lock). The service refuses it
...
# Public branch threshold retained here because callers and tests may tune it.
CHUNKED_UPLOAD_MIN_BYTES = 20 * 1024 * 1024

from box_operations import _BoxOperationsMixin, resolve_case_folder

class BoxClient(_BoxOperationsMixin):
