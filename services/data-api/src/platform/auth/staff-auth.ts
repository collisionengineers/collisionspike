/**
 * services/data-api/src/platform/auth/staff-auth.ts — Entra JWT validation + app-role authz.
 *
 * Validates Bearer tokens via the tenant JWKS and enforces the CollisionSpike.User /
 * .Superuser app roles. The earlier .Admin role remains accepted as the same privilege
 * because existing tokens can carry that role until they expire.
 *
...
// A token minted before the role rename can still carry 'CollisionSpike.Admin'. Treat either as
// superuser so renaming the app-role cannot lock out an assigned user before that token expires.
//
...
// fix exists to run. This earlier-role acceptance stays DELIBERATELY for any cached
