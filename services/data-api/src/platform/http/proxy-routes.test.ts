/**
 * services/data-api/src/platform/http/proxy-routes.test.ts — enrichLocationRequest photo-bytes trust boundary (TKT-077).
 *
...
vi.mock('../../features/evidence/bytes.js', () => resolver);
/* proxy.ts registers routes at import; stub the host + the other libs it pulls in. */
vi.mock('@azure/functions', () => ({ app: { http: () => {} } }));
vi.mock('../auth/staff-auth.js', () => ({ withRole: (_r: string, h: unknown) => h }));
vi.mock('../../features/settings/gates.js', () => ({ gates: {} }));
vi.mock('./service-client.js', () => ({ callLocationSuggest: vi.fn(), callParser: vi.fn() }));

import { enrichLocationRequest } from './proxy-routes.js';
