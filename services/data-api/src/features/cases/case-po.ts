/**
 * services/data-api/src/features/cases/case-po.ts — the shared advisory-locked Case/PO mint.
 *
 * ONE implementation of the per-(principal, year) sequence allocation used by BOTH
 * the manual-intake write path (features/cases/create-routes.ts) and the automated
 * email-intake resolve (features/cases/internal-resolution-routes.ts), plus the
 * provider API intake channel (features/providers/intake-route.ts).
 *
