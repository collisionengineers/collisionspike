...
vi.mock('../../shared/audit.js', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../../shared/audit.js')>()),
  actorFromClaims: () => 'staff-1',
  writeAuditStrict: audit.strict,
}));
const locks = vi.hoisted(() => ({ lock: vi.fn() }));
vi.mock('./mutation-locks.js', () => ({ lockCaseForMutation: locks.lock }));
const stores = vi.hoisted(() => ({ blob: vi.fn(), validate: vi.fn(), box: vi.fn() }));
vi.mock('../evidence/blob-store.js', () => ({ deleteEvidenceBytes: stores.blob }));
vi.mock('../../platform/http/service-client.js', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../../platform/http/service-client.js')>()),
...
const { FunctionCallError } = await import('../../platform/http/service-client.js');
