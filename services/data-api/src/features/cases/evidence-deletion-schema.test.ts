...
const schemaRoot = fileURLToPath(new URL('../../../../../database/', import.meta.url));
...
describe('single-image deletion schema safety', () => {
  it('gives every child FK to evidence a safe delete action', () => {
    ...
    expect(schema('baseline/195_staff_evidence_upload.sql')).toMatch(
  ...
  it('keeps owner-bypass protection and the scoped delete policy in fresh and delta DDL', () => {
    for (const sql of [
      schema('baseline/900_constraints.sql'),
      schema('migrations/2026-07-13-tkt160-evidence-deletion.sql'),
    ]) {
    ...
    for (const sql of [
      schema('baseline/205_evidence_deletion.sql'),
      schema('migrations/2026-07-13-tkt160-evidence-deletion.sql'),
    ]) {
