...
vi.mock('../cases/inspection-prefill.js', () => ({
  isPrefillApplicable: () => false,
  prefillImageBasedInspection: vi.fn(async () => false),
}));
vi.mock('../cases/overview-chase.js', () => ({ maybeSuggestOverviewChase: vi.fn(async () => false) }));
...
await import('../cases/register.js');
await import('../inbound/routes.js');
