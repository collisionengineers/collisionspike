...
vi.mock('../../platform/db/client.js', () => ({ assertStaffRlsContext: vi.fn(async () => undefined) }));
vi.mock('@cs/domain/gates', () => ({ gates: { mcpServer: () => true } }));
vi.mock('@cs/domain', () => ({
  agentCapabilities: () => [],
  capabilityByName: () => undefined,
}));
vi.mock('./chat-routes.js', () => ({ execTool: vi.fn() }));
...
const sessionMocks = await import('../cases/mcp-session.js');
