/**
 * services/data-api/src/features/assistant/chat-routes.ts — AI chat helper (TKT-060 / TKT-066 / TKT-069).
 *
...
import { runChat, type ChatMessage, type ToolDef, type ToolExecutor } from './chat-client.js';
import { computeAgingExceptions } from '../cases/dashboard-metrics.js';
import { archiveConfigured, archiveLookup } from '../archive/lookup.js';
import {
  CASE_SELECT,
  TWIN_TERMINAL,
  filterQueue,
...
const BASE_TOOL_NAMES = new Set(['lookup_case', 'count_cases_by_status', 'search_inbound']);
...
export function toolsForRequest(): ToolDef[] {
  ...
  const selected = v2 ? reads : reads.filter((c) => BASE_TOOL_NAMES.has(c.name));
...
export function buildExecutor(proposals: ProposedAction[]): ToolExecutor {
  return async (name, args) => {
    if (name === 'propose_action') {
      ...
      // schemas do not support a root union. Keep the top-level input during rollout.
      const proposed =
