#!/usr/bin/env python3
"""Real-email evaluation harness for the deterministic email classifier.
...
scores the result against a hand-labelled expected {category, subtype} per item. It is
separate from the parser's rule-level regression suite and the synthetic fixtures under
``tests/fixtures/email/triage``.
...
EVIDENCE_RESOLVER_DIR = REPO_ROOT / "tests" / "fixtures" / "resolvers"

if str(EVIDENCE_RESOLVER_DIR) not in sys.path:
    sys.path.insert(0, str(EVIDENCE_RESOLVER_DIR))

from evidence_resolver import resolve_evidence  # noqa: E402

# ---- locate + import the vendored engine (READ-ONLY import; the parser source is never
...
try:
    ...
    sys.stderr.write(
        ...
        "  Install the parser requirements: python -m pip install "
        "-r services/functions/parser/requirements.txt "
        "-r services/functions/parser/requirements-dev.txt extract-msg\n"
        f"  ({exc})\n"
    )
    raise SystemExit(2)
...
# Mirrors services/functions/parser/function_app.py's `_strip_html` (the server-side HTML->text
...
_FIELD_TO_PARAM = {
    ...
    # The orchestration service's OPEN-CASE match outcome (one|none|ambiguous) — like provider_match_state
...
def load_manifest(manifest_path: Path) -> list[dict[str, Any]]:
    ...
    # Its directory is ignored; see README.md "Local overlay".
    overlay_path = SCRIPT_DIR / "local" / "eval-overlay.json"
...
def resolve_manifest_file(item: dict[str, Any]) -> Path | None:
    """Resolve tracked catalog entries while also accepting local overlay files."""

    rel_file = str(item.get("file") or "")
    direct = (REPO_ROOT / rel_file) if rel_file else None
    if direct is not None and direct.is_file():
        return direct
    evidence_sha256 = str(item.get("evidence_sha256") or "")
    if evidence_sha256:
        try:
            return resolve_evidence(sha256=evidence_sha256)
        except FileNotFoundError:
            return direct
    if rel_file:
        try:
            return resolve_evidence(original_path=rel_file)
        except FileNotFoundError:
            return direct
    return None
...
def evaluate(items: list[dict[str, Any]], taxonomy: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for item in items:
        ...
        file_path = resolve_manifest_file(item)
...
def main() -> int:
    ...
    parser.add_argument("--taxonomy", choices=["v1", "v2"], default="v2", help="score against expected_v1 or expected_v2 (v2 falls back to v1 per item when absent). Default v2 tracks the in-tree engine; use --taxonomy v1 only to reproduce the baseline-v1.json reference result.")
