#!/usr/bin/env python3
"""Deterministic-versus-model A/B harness for the labelled email corpus.
...
(`services/orchestration/src/adapters/aoai.ts` uses the orchestration app's managed
...
MODEL CONTRACT — mirrors `services/orchestration/src/adapters/aoai.ts`:
...
`@cs/domain`'s codecs use (`packages/domain/src/data/code-tables/
...
try:
    ...
    sys.stderr.write(
        "ERROR: could not import run_eval.py from scripts/evaluation/email/ — this script reuses its\n"
...
CODE_TABLE_PATH = (
    REPO_ROOT / "packages" / "domain" / "src" / "data" / "code-tables" / "inbound-email-classification.json"
)


def load_taxonomy() -> tuple[list[str], list[str]]:
    """Returns (category_names, subtype_names) in the code tables' declared order."""
    data = json.loads(CODE_TABLE_PATH.read_text(encoding="utf-8"))
    by_name = {table["codeTableId"]: table for table in data["codeTables"]}
    categories = [o["name"] for o in by_name["inbound_category"]["options"]]
    subtypes = [o["name"] for o in by_name["inbound_subtype"]["options"]]
    return categories, subtypes


# ---- Default endpoint. LIVE_FACTS.json owns the verified live resource identity.
#      Override with --endpoint when evaluating an explicitly approved deployment;
...
# =============================================================================
# Request assembly — mirrors services/orchestration/src/adapters/aoai.ts's buildSystemPrompt /
...
def run_ab(
    ...
    for item in items:
        ...
        file_path = run_eval.resolve_manifest_file(item)
...
def main() -> int:
    ...
    parser.add_argument("--deployment", choices=DEPLOYMENT_CHOICES, default="gpt-5", help="the approved deployment name to call with --with-llm")
    parser.add_argument("--endpoint", default=DEFAULT_ENDPOINT, help=f"AOAI account endpoint (default: {DEFAULT_ENDPOINT})")
    ...
    try:
        ...
        sys.stderr.write(f"ERROR: could not load the taxonomy code tables ({CODE_TABLE_PATH}): {exc}\n")
        return 2
