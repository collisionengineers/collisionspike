...
Resolves the catalogued EVA export `docs/reference/fullevaexportinspectionaddresses.xlsx` and emits a
**PII-free**, deterministic suggestions CSV plus a per-provider run report.
...
Usage:
  python scripts/evaluation/inspection-corpus/build_corpus.py \
     ...
     [--out database/seeds/data/inspection-suggestions.csv] \
     [--report .artifacts/evaluation/inspection-corpus/provider-report.csv]
...
EVIDENCE_RESOLVER_DIR = os.path.join(REPO_ROOT, "tests", "fixtures", "resolvers")
if EVIDENCE_RESOLVER_DIR not in sys.path:
    sys.path.insert(0, EVIDENCE_RESOLVER_DIR)
...
def main(argv=None):
    ...
    ap.add_argument("--out", default="database/seeds/data/inspection-suggestions.csv")
    ap.add_argument("--report", default=".artifacts/evaluation/inspection-corpus/provider-report.csv")
    args = ap.parse_args(argv)

    source_xlsx = args.xlsx
    if not os.path.exists(source_xlsx):
        try:
            source_xlsx = str(resolve_evidence(original_path=args.xlsx))
        except FileNotFoundError:
            print(f"FATAL: source xlsx not found or catalogued: {args.xlsx}", file=sys.stderr)
            return 2

    sites, stats = build(source_xlsx)
