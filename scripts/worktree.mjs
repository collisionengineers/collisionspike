...
function setupComponents(meta, path) { execFileSync('npm', ['ci'], { cwd: path, stdio: 'inherit', windowsHide: true }); for (const component of meta.components.filter((x) => x.startsWith('python:'))) { const name = component.slice(7); const dir = name === 'ocr' ? join(path, 'ocr') : join(path, 'functions', name); const requirements = join(dir, 'requirements.txt'); if (!existsSync(requirements)) fail(`No locked requirements for ${component}.`); const python = process.platform === 'win32' ? 'py' : 'python3'; execFileSync(python, process.platform === 'win32' ? ['-3', '-m', 'venv', '.venv'] : ['-m', 'venv', '.venv'], { cwd: dir, stdio: 'inherit', windowsHide: true }); const pip = process.platform === 'win32' ? join(dir, '.venv', 'Scripts', 'python.exe') : join(dir, '.venv', 'bin', 'python'); execFileSync(pip, ['-m', 'pip', 'install', '-r', 'requirements.txt', '-r', 'requirements-dev.txt'], { cwd: dir, stdio: 'inherit', windowsHide: true }); } }
