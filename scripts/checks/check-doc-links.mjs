 ...
 *
 *   Usage:
 *     node scripts/checks/check-doc-links.mjs            # run links, orphans, leakage and data authority
 *     node scripts/checks/check-doc-links.mjs --quiet    # only print failures + summary
 *     node scripts/checks/check-doc-links.mjs --only=links     # broken relative links only
 *     node scripts/checks/check-doc-links.mjs --only=orphans   # unreachable docs/**.md only
 *     node scripts/checks/check-doc-links.mjs --only=leakage   # embedded live-number leakage only
 *
 ...
 *                   in LIVE_FACTS.json + docs/operations/live-environment.md.
 *
 *   Design note: this enforces docs/governance/documentation.md. It is intentionally
...
const REGISTRY_FILES = ['LIVE_FACTS.json', 'docs/operations/live-environment.md'];
...
const EXCLUDED_FROM_LEAKAGE = (rel) =>
  ...
  rel === 'docs/governance/documentation.md';
  ...
  for (const relFile of MD_FILES) {
    ...
    for (const { line, target, path } of linksIn(relFile, content)) {
      ...
      if (!existsSync(absResolved)) {
        failures.push({ file: relFile, line, target, resolved });
      }
    }
  }
  return failures;
...
if (!ONLY) {
  ...
  try {
    execFileSync(process.execPath, ['scripts/checks/check-repository-data-authority.mjs'], {
      cwd: ROOT,
...
if (failed) {
  console.log(
    'Fix: repair broken links, link or allowlist orphaned docs, and move any embedded\n' +
      'live number into LIVE_FACTS.json + docs/operations/live-environment.md (link the registry).',
  );
}
