...
const CANONICAL = 'docs/governance/repository-data-authority.md';
const REQUIRED = [
  '# Repository data authority',
  'complete bytes of',
  'raw image bytes may be sent',
  'unapproved service',
];
const DENY_PATTERNS = [
...
const SURFACES = [
  'AGENTS.md', 'CLAUDE.md', '.claude/agents', '.agents/skills', 'docs/adr', 'docs/governance',
  'docs/operations', 'docs/tickets/README.md', 'docs/tickets/BOARD.md',
...
function trackedFiles(root = ROOT) {
  return execSync('git ls-files', { cwd: root, encoding: 'utf8' })
    ...
    .filter((file) => ![
      'scripts/checks/check-repository-data-authority.mjs',
      'scripts/checks/check-repository-data-authority.test.mjs',
    ].includes(file));
}
...
export function run(root = ROOT) {
  ...
  const configPath = resolve(root, 'scripts/checks/repository-data-authority-allowlist.json');
