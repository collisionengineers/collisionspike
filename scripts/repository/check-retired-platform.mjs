...
const BINARY_EXTENSIONS = new Set([
  ".7z", ".avif", ".bmp", ".doc", ".exe", ".gif", ".gz", ".ico", ".jpeg", ".jpg",
...
const signatureDocument = JSON.parse(
  await readFile(new URL("./retired-platform-signatures.json", import.meta.url), "utf8"),
);
if (signatureDocument.encoding !== "base64-utf8") {
  throw new Error("Unsupported signature encoding");
}

const signatures = signatureDocument.signatures.map(({ id, value }) => ({
  id,
  text: Buffer.from(value, "base64").toString("utf8").toLowerCase(),
}));

function signatureIdsFor(value) {
  const normalized = value.toLowerCase();
  return signatures
    .filter((signature) => normalized.includes(signature.text))
...
async function main() {
  ...
  if (options.json) {
    ...
    process.stdout.write(`Retired-platform check: ${files.length} tracked files scanned.\n`);
