...
# Usage: scripts/database/cutover/tkt-009-window.sh [plan]
#        PLAN005_READONLY_PREFLIGHT_APPROVED=true scripts/database/cutover/tkt-009-window.sh inspect
#        PLAN005_READONLY_PREFLIGHT_APPROVED=true scripts/database/cutover/tkt-009-window.sh backup <absolute-dump-path>
#        <full cutover environment> scripts/database/cutover/tkt-009-window.sh phase-a|cutover <exact-release-worktree>
set -euo pipefail
...
SET ROLE csadmin;
\i ${TARGET}/database/migrations/2026-07-13-tkt009-outlook-message-link.sql
\i ${TARGET}/database/migrations/2026-07-13-tkt009-outlook-link-backfill-ledger.sql
RESET ROLE;
${inspect_sql}
SQL
    ;;
  cutover)
    psql "$CONN" -v ON_ERROR_STOP=1 <<SQL
SET ROLE csadmin;
\i ${TARGET}/database/migrations/2026-07-13-tkt009-mailbox-dedup-cutover.sql
RESET ROLE;
${inspect_sql}
SQL
    ;;
