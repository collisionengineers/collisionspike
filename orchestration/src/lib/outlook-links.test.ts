...
describe('findHistoricalMessageLink', () => {
  it('uses a mailbox-qualified exact filter and returns one immutable tuple', async () => {
    ...
    await expect(findHistoricalMessageLink('desk@collisionengineers.co.uk', '<same@example.test>'))
      .resolves.toEqual({
        status: 'resolved',
  ...
  it('abstains when Graph returns two exact hits', async () => {
    ...
    await expect(findHistoricalMessageLink('desk@example.test', '<same@example.test>'))
