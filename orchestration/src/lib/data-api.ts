...
export interface DeferredArchiveHoldingIntake {
  id:string;sourceMessageId:string;vrm:string;rootFolderId:string;claimToken:string;
...
export type ArchiveHoldingClaim =
  ...
  | { kind: 'claimed'; holdingId: string; claimToken: string; mode: 'rename' | 'merge';
      holdingFolderId: string; canonicalFolderId: string; casePo: string; files: ArchiveHoldingFile[] };
...
export const dataApi = {
  ...
  reserveArchiveHoldingIntake(payload:{
    vrm:string;rootFolderId:string;sourceMessageId:string;claimToken:string;
    files:Array<{filename:string;contentType:string;size:number;blobPath:string;sha256:string}>;
  ...
  registerArchiveHolding(payload: {
    vrm: string; rootFolderId: string; boxFolderId: string; sourceMessageId: string; claimToken: string;
    files: Array<{ filename: string; contentType: string; size: number; blobPath: string; sha256: string }>;
  }): Promise<{ holdingId: string; boxFolderId: string; files: ArchiveHoldingFile[]; deferred:boolean;replayed:boolean }> {
    return request('POST', '/api/internal/archive-holding/register', payload);
  },

  stampArchiveHoldingUpload(fileId: string, payload: {
    claimToken: string; boxFileId: string; boxFileUrl: string; boxSha1?: string;
  ...
  claimDeferredArchiveHoldingIntakes(claimToken:string,limit=10):Promise<{intakes:DeferredArchiveHoldingIntake[]}>
  ...
  failDeferredArchiveHoldingIntake(id:string,payload:{claimToken:string;error:string}):Promise<void>{
  ...
  checkpointArchiveHoldingFile(holdingId: string, fileId: string, payload: {
    claimToken: string; kind: 'moved' | 'deduplicated'; canonicalFileId: string;
    canonicalFileUrl: string; sourceRetired: boolean;
  }): Promise<{ updated: boolean }> {
    return request('POST', `/api/internal/archive-holding/${encodeURIComponent(holdingId)}/files/${encodeURIComponent(fileId)}/checkpoint`, payload);
  },

  finalizeArchiveHolding(holdingId: string, payload: {
    caseId: string; claimToken: string; folderId: string; folderUrl: string;
  }): Promise<{ adopted: number }> {
    return request('POST', `/api/internal/archive-holding/${encodeURIComponent(holdingId)}/finalize`, payload);
  },

  failArchiveHoldingAdoption(holdingId: string, payload: { claimToken: string; error: string }): Promise<void> {
