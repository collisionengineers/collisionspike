...
import { graphFetch, odataQuote } from './graph.js';
...
export type HistoricalLinkLookup =
...
/**
 * Historical read-only recovery. The RFC Internet-Message-Id is qualified by mailbox,
 * Graph is asked for immutable ids, and >1 exact hit abstains instead of guessing.
 */
export async function findHistoricalMessageLink(
  sourceMailbox: string,
  internetMessageId: string,
): Promise<HistoricalLinkLookup> {
  const mailbox = sourceMailbox.trim();
