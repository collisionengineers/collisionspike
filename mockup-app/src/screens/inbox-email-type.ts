/* ============================================================
   ...
   (show dismissed). Legacy `?category/?view/?triageState` params (old dashboard
   tiles, bookmarks) are migrated one-shot via migrateLegacyInboxParams.
   ============================================================ */
...
export interface MigratedInboxParams {
  emailType: EmailTypeFilter;
  showDismissed: boolean;
  /** True when a LEGACY param was consumed — the caller rewrites the URL once. */
  hadLegacy: boolean;
}

/**
 * One-shot legacy → new mapping for `?category/?view/?triageState` (pre-TKT-054
 * dashboard tiles + bookmarks): `category=X` → type=X; `view=all` or
 * `triageState=dismissed` → dismissed shown; every other legacy value folds into
 * the single list (New rows are already visible + amber). New-scheme params win
 * when both are present. `?peek=` is deliberately NOT consumed here.
 */
export function migrateLegacyInboxParams(params: URLSearchParams): MigratedInboxParams {
  const legacyCategory = params.get('category');
  const legacyView = params.get('view');
  const legacyTriage = params.get('triageState');
  const hadLegacy = legacyCategory != null || legacyView != null || legacyTriage != null;

  const emailType = params.get('type') != null
    ? parseEmailType(params.get('type'))
    : parseEmailType(legacyCategory);
  const showDismissed = params.get('dismissed') != null
    ? params.get('dismissed') === '1'
    : legacyView === 'all' || legacyTriage === 'dismissed';

  return { emailType, showDismissed, hadLegacy };
}
