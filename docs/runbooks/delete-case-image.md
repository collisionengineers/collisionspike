...
1. Apply `migration/assets/schema/deltas/2026-07-13-tkt160-evidence-deletion.sql` to Postgres.
2. Publish `functions/box-webhook/` so `GET|DELETE /api/box/files/{fileId}?folderId=...` exists.
3. Publish `api/` with `deleteCaseImage` and the replay/mirror/classification guards.
4. Build and publish `mockup-app/`.
...
npm run test --workspace mockup-app
python -m pytest -q functions/box-webhook/tests
node verify-all.mjs
```

The focused acceptance suites are `api/src/functions/evidence-delete.test.ts`,
`api/src/functions/internal-evidence-dedup.test.ts`, `api/src/lib/functions-client-box.test.ts`,
`mockup-app/src/data/rest-client.test.ts`, `functions/box-webhook/tests/test_scope_lock.py`, and
`functions/box-webhook/tests/test_file_deletion_routes.py`.
