...
> re-embedding a count. Last live change: **2026-07-02T10:55Z** — rules-engine-v2 **Phase-0 deploy**: orch +
> parser redeployed (classify contract pass-through — `attachment_filenames` sent, `body_jobref` +
> `conversationId` captured; vendored engine re-cut at sibling tag `engine-v2.1`), function counts unchanged
> (orch 51, parser 3, api 65 untouched); live `/classify-email` probe returned `body_jobref` + the
