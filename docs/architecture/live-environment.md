...
> re-embedding a count. Last live change: **2026-07-10T21:10Z** — the **operator-authorized activation
> batch** (config + Entra only — **no deploy**, function counts unchanged): (1) **`BOX_REG_FOLDER_ENABLED=true`**
> on `cespk-orch-dev` (TKT-034 step-2, readback-verified — the reg-keyed Box holding folder for unmatched
> image emails is now ACTING; the consumer `imagesUnmatched` was already live). (2)
> **`RETRO_BOX_ARCHIVE_ROOT_IDS=4077648161`** on `cespk-api-dev` **only** (TKT-107 config half — the root
> re-confirmed read-only first via `box folders:get`: **"Collision Engineers"**, root-level, 7 top-level
> entries, ~494.7 GB; on the api its sole reader is the suggest-only assistant **`archive_lookup`**
> (`archiveConfigured()` now true); deliberately **NOT set on orch** — the minting R2 reconstruction rung
> stays dark pending the [gated.md D11](../gated.md) Case/PO sequence alignment). (3) **TKT-110 MCP
> Flow A**: NEW Entra app-registration **`CollisionSpike MCP Client`** (appId
> `88589edc-527a-4d84-83da-a2ddb8664f08` — public client, Auth-Code + PKCE, loopback redirect, **no
> secret**, sole delegated permission = the API's `access_as_user` with tenant consent granted, **no**
> Flow B app-roles) + **`MCP_SERVER_ENABLED=true`** on `cespk-api-dev` — unauthenticated `POST /api/mcp`
> still **401s** fail-closed (`Missing bearer token`); the remaining verify item is the live Flow A
> interactive-client probe. (4) **Staff app-roles (C1 partial close)**: `CollisionSpike.User` assigned on
> the API SP to the operator-directed interim set **desk@ + info@ + engineers@** (idempotency-checked
> first; digital@'s Superuser untouched — the API SP now carries **4** assignments; ids in
> [`LIVE_FACTS.json`](../../LIVE_FACTS.json) `staffAppRoleAssignments` + `mcpAppRegistration`). Prior
> change: **2026-07-10T17:55Z** — **TKT-089 REOPEN fix (non-vehicle
...
> TKT-143 provider/VRM stem threading); **parser republished — still 4** (`/extract-images` accepts
> optional `provider`/`vrm` stem tokens — TKT-143; vendored engine untouched at `engine-v2.11`);
> **SPA redeployed** (200 + strict CSP re-verified — Action-logs plain-English primary lines + the
> "Technical details" disclosure). **TKT-138 root-caused (read-only)**: the API app registration AND its
> service principal both read `Engineer/User/Superuser` (Superuser keeping id `5b356d4c`), and the
> operator's only API app-role assignment targets that id — a fresh token can only mint
> `CollisionSpike.Superuser`; the observed `CollisionSpike.Admin` claims are stale pre-rename artifacts
> (no directory fix exists to run; the deliberate legacy-accept stays documented in `api/src/lib/auth.ts`).
> Prior change: **2026-07-09T06:20Z** — the **PLAN-003 lifecycle wave**
...
> [gated.md D11](../gated.md). **Current boundary (2026-07-13):** a normal single-case stamp during
> ordinary trial handling is not bulk-cutover authority or proof. Production floor seeding, deterministic
> renumbering and Archive folder/root work remain blocked by TKT-178 until the signed/checksummed job
> spreadsheet, authenticated production EVA API and exact approved production Archive root/write scope all
> reconcile in the frozen approved ledger. Prior change: **2026-07-04T17:55Z** — the **retro-reconstruction activation**
...
> This is a dated activation record; its former Viewer-plus-sequence gate is superseded by TKT-178 and does
> not authorize production reconstruction, root retarget or Archive writes.
...
- **Assignment state (2026-07-10):** the API SP carries **4 app-role assignments** — `digital@`
  (**Superuser**, pre-existing) plus the **operator-directed interim staff set** `desk@` + `info@` +
  `engineers@` (each **`CollisionSpike.User`**, assigned 2026-07-10 — ids in
  [`LIVE_FACTS.json`](../../LIVE_FACTS.json) `staffAppRoleAssignments`). Any **other** signed-in user
  still reaches the API and **403s** until an admin assigns them a role ([gated.md C1](../gated.md)).
- **MCP Flow A client (2026-07-10, TKT-110/ADR-0023):** **`CollisionSpike MCP Client`** (appId
  `88589edc-527a-4d84-83da-a2ddb8664f08`) — a **public** client (Auth-Code + PKCE, loopback redirect,
  no secret) whose sole permission is the API's delegated **`access_as_user`** scope (tenant consent
  granted). Interactive MCP clients use it to mint **delegated staff tokens for the existing API
  audience** (`fa2fb28c…`) against `POST /api/mcp`; authorization stays at the Data API (`withRole`).
...
1. **Email intake is LIVE IN TESTING — now on the production mailbox set.** `cespk-orch-dev` is live with
   **3 Graph PUSH subscriptions** over the production set **info@ + engineers@ + desk@** (all Exchange-RBAC-scoped;
   ...
   - ✅ **Subscription prune implemented (2026-07-05).** `runSubscriptionMaintenance`
     (`orchestration/src/lib/subscriptions.ts`) creates missing intake subscriptions, renews current ones and
     prunes subscriptions whose mailbox/folder is no longer configured. The earlier manual digital@ cleanup
   ...
   - ✅ **Renewal RESOLVED (2026-06-29, AIE Wave A).** Root cause: a plain timer trigger isn't woken on Flex
     ...
     renewed to 2026-07-06T10:19Z. **Evidence gap:** current `graph-renewal-success` text is an ordinary trace
     shared by durable, manual-HTTP and timer paths. Operational diagnosis can pin `operation_Name` to the
     durable activity, but TKT-178 certification requires the planned versioned source-bearing custom event.
...
Infisical). This describes ordinary current handling only: drag-drop and a test parity result do not satisfy
TKT-178, whose production cutover requires an authenticated contract-verified production EVA API result in
the same approved ledger as the signed spreadsheet and production Archive inventory. See
[eva-sentry-api.md](./eva-sentry-api.md).
