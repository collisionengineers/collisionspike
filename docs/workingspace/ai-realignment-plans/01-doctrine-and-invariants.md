...
1. **AI proposes; deterministic code disposes.** A model output is evidence, never an action.
   Stage C is "a suggestion writer, never an actor" ([ADR-0019 §3](../../adr/0019-triage-policy-stage-split.md));
   the assistant write tier is propose → confirm → execute ([ADR-0024](../../adr/0024-assistant-write-tier-confirmation-protocol.md));
...
2. **Abstain on ambiguity; nothing is silently dropped.** Every email is categorised (spam lands
   `other`, not a pre-filter — [ADR-0015](../../adr/0015-email-triage-inbox-management.md));
   uncorroborated promotion is a bug (the 2026-06-29 corroboration gate); ambiguous matches go
   to a human, never a guess ([ADR-0010](../../adr/0010-dedup-reference-disambiguated-no-time-window.md)).
3. **Identifier discipline.** One VRM canonicaliser ([ADR-0025](../../adr/0025-shared-capability-registry.md));
...
4. **Suggest-first promotion ladder.** Every new behaviour ships behind its own default-off gate,
   ...
   plus live staff confirmations ([ADR-0019 §4](../../adr/0019-triage-policy-stage-split.md)).
...
5. **Authorization is enforced at the Data API, never at an AI layer.** Registry flags are
   advisory; `withRole`/RLS/audit are the enforcers ([ADR-0023](../../adr/0023-mcp-server-hosting-and-auth.md),
...
7. **Per-provider consent + global kill switch.** `work_provider.ai_allowed` and the provider
   ...
   ([ADR-0019 §3](../../adr/0019-triage-policy-stage-split.md), `CONTEXT.md` → Provider Automation Mode).
...
9. **Honest PII posture.** `scrubPii` is a precision-over-recall **pre-scrub**, not
   de-identification ([ADR-0024](../../adr/0024-assistant-write-tier-confirmation-protocol.md));
   ...
   [data-protection.md §6a](../../architecture/data-protection.md) — never inherited from a
...
|---|---|---|---|---|
| **T1 — in-app assistant** | `/api/assistant/chat` + read tools + write tier | A staff human, per action | Propose → confirm → execute; ETag/If-Match; destructive = human-only, never proposable ([ADR-0024](../../adr/0024-assistant-write-tier-confirmation-protocol.md)) | Live (chat, toolset v2, write tier flipped) |
...
| **T3 — external agents** | MCP (`POST /api/mcp`) | No human — hence read-only | Reads only (Flow A); autonomous writes deferred behind the ADR-0023 bar (agent app-role + signed commit token + ETag) | Read-only live; image-ingest lane dark ([mcp-image-ingestion.md](../../architecture/mcp-image-ingestion.md)) |
...
- **Q3 — Case/PO as an inbound signal.** aifirstplan removes it entirely; [ADR-0015 §5](../../adr/0015-email-triage-inbox-management.md)
...
- **Q5 — Taxonomy growth.** The emailevals tree names real leaves the live 9/16 taxonomy cannot
  ...
  choiceset changes with the ADR-0015 deploy-order discipline, adopted when adjudicated corpus
