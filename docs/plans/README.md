...
docs/plans/
  ...
  phase-5-ocr-and-scale/             ocr-strategy (5a) · image-classification-ai (5b) · valuation (5c)
...
| [phase-5-ocr-and-scale/valuation.md](./phase-5-ocr-and-scale/valuation.md) | Staff-triggered valuation (direct-REST-wrapper Function → Companion PDF as `Evidence(kind=valuation)`, gated `VALUATION_ENABLED`). | **Phase 5c** (Valuation) |
...
| [inspection_address_suggestions_fix_a8b283cb.plan.md](./inspection_address_suggestions_fix_a8b283cb.plan.md) | Repair the inspection-address **suggestions** pipeline end-to-end: rebuild the offline corpus from the full EVA export, fix the API/SPA suggestion surfaces + location-assist wiring, reseed live — ADR-0013 (no runtime matcher) stays binding. | **Phase 4a** (ADR-0016 _Proposed_) |
| [phase-8-inbox-management/README.md](./phase-8-inbox-management/README.md) | Classify **every** inbox email into the operator taxonomy (receiving-work / query / other); deterministic `/classify-email` MVP + `cr1bd_inboundemail` triage table → query-queue UI → gated LLM. _Power-Platform-era build (decommissioned); the Azure-era continuation is [rules-engine-v2-build.md](./phase-8-inbox-management/rules-engine-v2-build.md) below._ | **Phase 8** (ADR-0015 _Proposed_) |
...
| [rules_engine_v2_plan_9ba034c4.plan.md](./rules_engine_v2_plan_9ba034c4.plan.md) | **Azure-era continuation of Phase 8.** Context-aware triage-policy upgrade: an open-case ref-gate + thread correlation + Image-Source intermediary resolution (ADR-0011) in `packages/domain`, taxonomy v2 (`case_update`/`cancellation`) behind a strict deploy order, a real-email eval harness, and the gated LLM suggestion assist wired to the Foundry deployments — suggest-first, PII-scrubbed, operator-gated. Distilled build checklist: [phase-8-inbox-management/rules-engine-v2-build.md](./phase-8-inbox-management/rules-engine-v2-build.md). | **Phase 8** (Azure-era realization; ADR-0019) |
...
| **Phase 5c** — Valuation — **M3** | **valuation** |
