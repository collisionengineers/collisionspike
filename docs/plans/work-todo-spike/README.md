# work-todo-spike — operator stubs + research packs

> **Advisory only.** Live facts (counts, gates, mailbox set, function names) live in the registry
> [`LIVE_FACTS.json`](../../../LIVE_FACTS.json) /
> [live-environment.md](../../architecture/live-environment.md). **Authoritative delivery state** is on
> [docs/tickets/BOARD.md](../../tickets/BOARD.md) — one ticket per item below.

Fan-out research + operator notes that fed the **TKT-001…020** cohort. Research packs stay here; each
ticket links its pack via `research-link`. When a ticket is **done**, the operator stub gets a one-line
pointer at the top (distillation convention — see [plans/README.md](../README.md)).

## Cohort A index (stub → ticket → status)
