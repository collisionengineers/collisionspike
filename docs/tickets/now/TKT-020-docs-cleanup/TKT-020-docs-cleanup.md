...
research-link: docs/tickets/now/TKT-020-docs-cleanup/evidence/operator-note-2026-07-15.md
plan: PLAN-006
---
...
## Problem
The repository has overlapping source, evidence, documentation, planning and agent-authority roots. Stale paths and duplicate narratives force an agent to infer which source is current, while accumulated retirement material makes a pre-release system appear to have two active implementations.
...
- The operator supplied a locked target structure, authorized deletion and movement across the repository, required complete removal of retired-platform material, and protected the four workingspace files from content edits.
...
- **A3.** The final tracked tree contains zero retired-platform technology names, implementation names, identifiers, URLs, commands, old prefix-based logical names, compatibility paths or explanatory history.
...
## Research
Distilled from the operator's 2026-07-15 repository-organization and hygiene direction. Earlier TKT-020 material is retained in Git history, not in the active tree.
