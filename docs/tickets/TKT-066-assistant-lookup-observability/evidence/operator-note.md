...
- **Assistant lookup**: `POST /api/assistant/chat` requests all returned **200** (5–13s). App
  ...
  causes are in code:
  - `api/src/lib/aoai-chat.ts` line 174: tool exceptions become `{error}` fed to the model
    **with no logging** — the "internal error" the model reported is invisible in telemetry.
  - `api/src/functions/assistant.ts` line 88: `lookup_case` does `c.vrm ILIKE '%YT13 UTV%'` but
    intake stores VRMs **compacted** (`YT13UTV` — `extractVrm` strips spaces), so a spaced VRM
    can never match.
- **Email body**: `orchestration/src/functions/activities/fetchMessage.ts` line 137 collapses ALL
...
- **HD4110**: `packages/domain/src/domain/vrm-filter.ts` — the LOOSE dateless rule accepts
  ...
  districts.
- **Search**: `mockup-app/src/components/AppShell.tsx` line 438–444 — the SearchBox only
...
In `api/src/functions/assistant.ts` and `api/src/lib/aoai-chat.ts`:
...
- Resilience: retry a failed tool query once (covers Postgres cold-connect within the 5s
  `connectionTimeoutMillis` in `api/src/lib/db.ts`).
...
- New pure util in `packages/domain/src/domain` (e.g. `email-body-clean.ts`): preserve newlines
  ...
  common markers. Unit-tested on real samples from `test-cases-and-data/`.
- Use it in `fetchMessage.ts` (and `orchestration/src/lib/retro-envelope.ts`) to build
...
- Deploy api + orch + SPA per `docs/azure/deploy.md`; verify live with a YT13 UTV assistant
