...
(DPIA-gated image-egress, docs/gated.md §F.7). Built DARK on `feat/plan-001-vision-family`.
...
## Files touched
**New**
- `api/src/lib/image-analysis.ts` — the PURE staged pipeline (`runImageAnalysis`) over injectable
...
- `api/src/lib/image-analysis-adapters.ts` — the NETWORK-backed adapters the route injects: keyless-MI
...
- `api/src/functions/image-analysis.ts` — the gated route `POST /api/cases/{id}/image-analysis/generate`:
...
- `api/src/lib/image-analysis.test.ts` — the offline acceptance suite (10 tests): full staged sequence,
  graceful degradation per stage, reg tri-state (F3), "detected VRM ≠ case identity", VLM schema/parse.
- `migration/assets/schema/deltas/2026-07-08-image-analysis-suggestion-types.sql` — additive/idempotent
...
- `api/src/lib/functions-client.ts` — added the Data-API-side `callPlateOcr` (OCR_FN_URL/KEY) twin of orch's.
- `api/src/lib/audit.ts` — added `image_analysis_generated: 100000052` (continues from `agent_write` 100000051).
- `migration/assets/schema/000_enums_lookups.sql` — canonical seed of audit code 100000052 (keeps a fresh
  rebuild in sync with the delta).
- `api/src/index.ts` — registers the new route module.
...
`orchestration/src/lib/image-classify.ts` (the live TKT-064 auto-writer) — reconciliation is TKT-088/112 and
...
image-egress residency cost that DPIA-gates the live flip (annotated in docs/gated.md §F.7). The address
...
**Proof (offline, G5 repo-data):** `npm --prefix api test -- image-analysis` (10/10). Full suites green
