...
- `functions/enrichment/analysis.py` — current estimator implementation to reconcile, not assume authoritative.

## Proposed change
PROPOSED (not built): establish one versioned vehicle-data domain package and service contract, migrate every caller to it, then implement and chronologically backtest an auditable robust estimator.
...
- A repository and sibling-integration scan proves that only one handwritten estimator remains; obsolete Cloud Run/Function implementations are retired, redirected, or mechanically generated from the canonical source with parity tests.
