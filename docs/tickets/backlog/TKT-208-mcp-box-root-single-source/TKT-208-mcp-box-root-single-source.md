...
research-link: docs/tickets/backlog/TKT-208-mcp-box-root-single-source/evidence/operator-note.md
plan: PLAN-004
---
...
- two hard-coded TypeScript constants (`MCP_IMAGE_INGEST_TEST_ROOT_ID` in
  `api/src/functions/mcp-image-ingestion.ts` and the literal in
...
- The Python `box-webhook` `BOX_ALLOWED_ROOT_ID` gap is captured in the Box activation checklist
  (`docs/azure/box-activation.md`) so an operator cannot enable the gate with the façade root unset/wrong.
