...
- `mockup-app/src/screens/AddEvidence.tsx:123-128` stores selected `File` objects in local state.
- `mockup-app/src/screens/AddEvidence.tsx:131-133` implements `attach()` as navigation to the selected case only; it never reads `files` or calls an upload transport.
