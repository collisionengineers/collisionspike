...
| A5 — canonical roles only | Contract/snapshot tests reject legacy Admin and Engineer assignment options/writes. | Live enterprise-app assignment inspection shows only canonical User/Superuser values for approved genuine changes. | PENDING |
...
| A10 — stable ownership and unspoofable names | Claim/API tests reject client display spoofing and prove alias rename does not rewrite historical snapshots. | Two genuine staff actions show distinct trusted names; authorized detail resolves stable principals without raw-ID leakage elsewhere. | PENDING |
