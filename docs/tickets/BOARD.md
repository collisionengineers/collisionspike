...
| [TKT-093](./TKT-093-auto-attach-matched-emails/TKT-093-auto-attach-matched-emails.md) | Auto-attach matched emails + visibility + case_update misclass | **MISCLASS FIX LIVE + PROVEN 2026-07-07** (forward "Audatex attached" → `case_update`, was audit re-inspection). **Auto-attach BUILT + DEPLOYED DARK** (`TRIAGE_AUTO_ATTACH_ENABLED` off; case_po/job_ref exact-single only — VRM-only stays a suggestion per ADR-0010/0019; api+orch deployed). Inbox-list "may belong to · <Case/PO>" visibility built (api deployed). **Remaining:** SPA deploy + the gated live auto-attach flip. See [verification](./TKT-093-auto-attach-matched-emails/verification.md). |
