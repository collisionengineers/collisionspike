...
> confirmed rows byte-identical, firehose closed, QDOS/PCH now scoped). Remaining are operator live
> SPA click-throughs + `AZURE_MAPS_KEY` (proximity) + `LOCATION_ASSIST_AI_ENABLED` sign-off. Full narrative:
...
| [TKT-078](./TKT-078-location-assist-ai-escalation/TKT-078-location-assist-ai-escalation.md) | Deeper photo-based location suggestion — AI reasoning escalation (gated) | **BUILT + DEPLOYED DARK 2026-07-06** — `ai_reasoning.py` (keyless AOAI gpt-5 vision, reasoning-model form, caps + telemetry) wired as a `deep=true` branch; gate `LOCATION_ASSIST_AI_ENABLED` default OFF, `build_reasoner()` returns None today; hidden UI button. Live flip operator-blocked (gated.md E2). See [verification](./TKT-078-location-assist-ai-escalation/verification.md). |
