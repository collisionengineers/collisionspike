...
| [TKT-025](./verify/TKT-025-inbox-source-filter/TKT-025-inbox-source-filter.md) | Mark + filter inbox by source mailbox | DEPLOYED live in the SPA bundle 2026-07-02 (toolbar mailbox-chip filter). Needs a live click-through. See [verification](./verify/TKT-025-inbox-source-filter/verification.md). |
...
| [TKT-031](./verify/TKT-031-misclass-client-chasing/TKT-031-misclass-client-chasing.md) | Client report-chaser misrouted to 'Other' | Eval-passing on the deployed engine (committed corpus, 2026-07-02); awaiting a live occurrence/probe to close. See [verification](./verify/TKT-031-misclass-client-chasing/verification.md). |
| [TKT-039](./verify/TKT-039-misclass-query-report-support/TKT-039-misclass-query-report-support.md) | Report-support request misclassified as new case | Eval-passing on the deployed engine (committed corpus, 2026-07-02); awaiting a live occurrence/probe to close. See [verification](./verify/TKT-039-misclass-query-report-support/verification.md). |
| [TKT-047](./verify/TKT-047-email-sigs-box/TKT-047-email-sigs-box.md) | Email signature images archived to Box in error | Non-inline raster floor DEPLOYED live on orch 2026-07-02; awaiting live proof on a real signature-bearing email. |
...
| [TKT-076](./verify/TKT-076-inspection-provider-scope-proximity/TKT-076-inspection-provider-scope-proximity.md) | Inspection suggestions ignore the provider and distance — real scoping + nearest-first | **DEPLOYED + data-proven 2026-07-06** (api 82). Server-side `provider_code` scoping + the labelled fallback KILLED the `!s.providerCode` firehose; proximity ordering (`api/src/lib/maps.ts` haversine + `distanceMiles`) is **now live** — `AZURE_MAPS_KEY` wired on the api app as a versioned KV ref 2026-07-06 (geocode smoke passed: ML6 8TA, M12 4AH → real lat/lon), degrading honestly to frequency ordering when a case has no parseable postcode. Live smoke: QDOS/PCH now present, 0 suggested rows without a provider_code. Live SPA click-through pending (bearer-gated). See [verification](./verify/TKT-076-inspection-provider-scope-proximity/verification.md). |
...
| [TKT-089](./verify/TKT-089-non-vehicle-images-box/TKT-089-non-vehicle-images-box.md) | Confirm non-vehicle images (signatures/logos) no longer captured/stored on Box | **Reclassified backlog → verify (2026-07-07)** — the PDF-lane fix is **BUILT + DEPLOYED on `main`**: `is_decorative()` 200×200 area floor (`functions/parser/cedocumentmapper_v2/application/service.py:401`, both PDF paths 433/453), unit-tested on this ticket's exact QDOS26004 `RJS_UnknownVRM` logo (`test_extract_images.py:161`, `count==0`), shipped via `aafeba1`. Remaining: the live audit + TKT-047 email-lane proof + backfill decision. **Caveat:** it's a *size* floor — a large logo above 200×200 still passes (audit must confirm/raise a follow-up). **Read-only verifier pass (2026-07-08):** no regression on any readable surface — 221 live signature-skip traces since the 07-02 deploy, parser serving the fixed engine, vehicle-photo recall intact; the DB/Box lane audit needs a non-read-only pass (firewall rule = mutation). See [verification](./verify/TKT-089-non-vehicle-images-box/verification.md). |
...
| [TKT-103](./backlog/TKT-103-tractable-reference-bug/TKT-103-tractable-reference-bug.md) | Tractable "768.00" wrongly captured as the reference number | P2 drop-note (2026-07-07): a monetary/estimate value taken as the provider reference on the Tractable layout (false-token family, cf. TKT-071/085); anchor the ref rule + eval pin. |
| [TKT-105](./backlog/TKT-105-remittance-payments-category/TKT-105-remittance-payments-category.md) | Remittance advice classified under payments/billing | P2 drop-note (2026-07-07): inbound remittance-advice emails have no home — route to a payments/billing category (sibling of TKT-037). Sat in the operator's `done/` area but was never ticketed. |
...
| [TKT-109](./backlog/TKT-109-image-based-provider-prefill/TKT-109-image-based-provider-prefill.md) | Pre-fill image-based inspections for image-led providers | Operator note: image-led providers should pre-fill Image Based Assessment; relates TKT-079/TKT-024. |
...
| [TKT-057](./blocked/TKT-057-ap-diminution-refinement/TKT-057-ap-diminution-refinement.md) | A real inbound diminution instruction email (grounds `D.` detection) + a standalone a.qdos inbound email if one exists; then the SPA case-type control for the review-time AP. refinement ([ADR-0021](../adr/0021-case-po-marker-taxonomy.md)). Also gated behind TKT-056's activation ladder. |
| [TKT-088](./blocked/TKT-088-image-role-classification-check/TKT-088-image-role-classification-check.md) | Operator decision on image-role classification (determination already made: it is unbuilt — TKT-064): build the auto-classifier now, keep manual with better UX, or defer. |
| [TKT-104](./blocked/TKT-104-tractable-api-integration/TKT-104-tractable-api-integration.md) | Tractable **developer docs** for the full API integration (in-app link generation + direct case ingestion). Deferred until the vendor supplies them; the received-email path (TKT-102) covers the immediate need. |
| [TKT-112](./blocked/TKT-112-image-writer-reconcile/TKT-112-image-writer-reconcile.md) | Reconcile the two image-classification writers | Blocked on TKT-088 operator decision: choose one image classification writer model before vision expansion. |
