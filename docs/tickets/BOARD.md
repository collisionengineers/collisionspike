...
| [TKT-059](./TKT-059-replay-wipe-rebuild/TKT-059-replay-wipe-rebuild.md) | Replay: wipe & rebuild derived data from full mailbox history | NEW (go-live sprint, 2026-07-04). Operator-approved WIPE & REBUILD ([GO_LIVE_SPRINT_PLAN.md](../../GO_LIVE_SPRINT_PLAN.md)) — 164 cases / 389 inbound_email were processed by since-fixed code (baseline captured 2026-07-04, `SET ROLE csadmin`). Build (P1) = `listMessagesSince` pager + keyed `POST /api/replay-backfill` Durable driver (dry-run + live, 3-way chronological merge, `replay-r1-<safeId>` namespace); execute (P3) = pg_dump + human-work export + Box holding move → wipe delta (DELETE not TRUNCATE CASCADE; `audit_event` kept; `case_po_floor` seeded from pre-wipe maxima) → re-ingest at production gate settings; verify (P3V) = manifest reconciles 100% + consistency gates + probe-cluster closure. Not started. |
| [TKT-062](./TKT-062-inspection-shortlist/TKT-062-inspection-shortlist.md) | Inspection-address picker returns entire corpus — add ranked shortlist | NEW (2026-07-04). `inspectionAddressSuggestions` (`api/src/functions/inspection.ts`) returns all `suggested%` rows with no `LIMIT` and falls back to the full corpus (2210 rows, confirmed live) on empty provider scope; the SPA renders them 1:1. Change: server-side rank (postcode outward-code > provider-scope > `suggestion_rank`/`frequency`/`last_seen_on`) + ~8-row cap + "Search all locations…" typeahead. Pure string ranking (ADR-0013), ADR-0016 metadata. |
| [TKT-063](./TKT-063-go-live-docs/TKT-063-go-live-docs.md) | Go-live runbook, readiness matrix & operator checklist | NEW (2026-07-04). Authoring `docs/plans/go-live/` (runbook, readiness-matrix, day0-smoke, rollback, support-playbook + operator checklist). Headlines the **Free-Trial→PAYG hard deadline** (~2026-07-18; quotaId `FreeTrial_2014-09-01`, oldest resource 2026-06-18) as the #1 operator action. |
...
| [TKT-060](./TKT-060-ai-chat-helper/TKT-060-ai-chat-helper.md) | AI chat helper — read-only Q&A assistant drawer | NEW (2026-07-04). Gated read-only Q&A drawer, distinct from TKT-015's suggestion layer. `POST /api/assistant/chat` streamed via fetch-reader, keyless `gpt-5` on `digital-3339-resource` behind a new `cespk-api-dev` MI Cognitive Services OpenAI User grant, four read-only tools, `AI_CHAT_ENABLED` default-off, RLS-scoped + audited + rate-limited, Fluent v9 Sparkles drawer off the AppShell header. |
| [TKT-061](./TKT-061-box-cli-webhook-e2e/TKT-061-box-cli-webhook-e2e.md) | Box CLI + FILE.UPLOADED webhook + sandboxed E2E | Box CLI installed (`@box/cli` 4.9.2) + JWT-authed on dev root `392761581105` (124 items) 2026-07-04. Remaining: create the `FILE.UPLOADED` webhook via `POST box/webhooks` → the `box-webhook` receiver, then a sandboxed upload→evidence→`box_upload_received` audit→status-reeval→SPA E2E + read-only mirror audit. File Request template id stays operator (Box-UI). |
## Backlog — not started
...
| [TKT-048](./TKT-048-no-image-previews/TKT-048-no-image-previews.md) | Inbox/case image previews not rendering | Orphan folder ticketed 2026-07-04. Evidence tiles render as coloured placeholder divs, never an `<img>` (`CaseDetail.tsx`, `ImageOrderList.tsx`); the Evidence model carries no preview bytes (mock `thumbColor` only). Needs a CSP-legal (`img-src 'self' data: blob:`) Data-API preview path. |
