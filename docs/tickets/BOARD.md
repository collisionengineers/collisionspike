...
| [TKT-145](./verify/TKT-145-caselink-evidence-backfill/TKT-145-caselink-evidence-backfill.md) | Accepted case_link on a previously-uncased email must backfill its evidence | Shipped + DEPLOYED 2026-07-10; proof RE-STAGED after the drain mooted the first (a mooted accept provably does NOT backfill — the accept UPDATE guards case_id IS NULL). Operator accepts suggestion 025c8ce2 on the uncased desk@ SW18EAY email → A.QDOS26034 (baseline 24 evidence rows). Cousin gap filed: drain-linked emails skip backfill (the queue+consumer can serve them; new-ticket candidate). → verify. |
...
| [TKT-141](./done/TKT-141-merged-twins-exclusion/TKT-141-merged-twins-exclusion.md) | Exclude merged/retired duplicate cases from twin counts and attention lists | VERIFIED-LIVE 2026-07-10 (fresh verifier after the same-day FAILED→retired-lock→re-retire cycle): PK20FWT badge=1 observed, retired rows out of every count yet openable ("Linked to instruction"), no over-suppression (genuine twins still chip), lock held through ~20 min live churn; root cause was TKT-131's re-evaluate. → done. |
...
| [TKT-144](./done/TKT-144-blob-sha256-backfill-dedup/TKT-144-blob-sha256-backfill-dedup.md) | Resolve the 214 blob-lane same-name duplicate evidence rows via a sha256 backfill | VERIFIED-LIVE 2026-07-10 (ticket-verifier, two-stage): evidence pack recomputed exactly offline; W2 data pass all six values exact (0 null-sha of 3,434; 0 dup groups; 108/108 twins; audits 70/36/2; 586 at run ts). Caveat carried: the 132-bucket rename-dup class needs its own ticket. → done. |
