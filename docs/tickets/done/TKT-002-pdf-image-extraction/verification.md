...
- Unit test: `mockup-app/src/components/ImageOrderList.test.ts`.
DB reads cross-checked against App Insights custom events.

## Pending / gaps
Registration-flagging of unsuitable photos needs `PLATE_OCR_ENABLED` (currently off). With OCR off the pipeline degrades to a generic note rather than detecting no-registration-visible images, so that half is NOT yet live. Gate state: see ../../architecture/live-environment.md.
