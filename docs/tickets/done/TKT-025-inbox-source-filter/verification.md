...
- The mock/dev fixture data (`mockup-app/src/data/mock-source.ts`) was updated to use the **live** three-mailbox set (`info@`/`engineers@`/`desk@collisionengineers.co.uk`) in place of the retired `digital@`/`enquiries@` addresses it previously carried, so the chip row demonstrates correctly in `npm run dev` — this has no bearing on live behaviour (the live API always returns whatever `GRAPH_INTAKE_MAILBOXES` actually ingested).
