...
VERIFIED-LIVE. The remittance sample classifies billing/payment_remittance live (taxonomy v3) with the payments lane beating the instruction-doc promotion that pre-fix minted a case; pin tkt105-remittance-advice green (payment_remittance 2/2; the invoice-REQUEST lane untouched 1/1); the SPA E-mail-type dropdown renders the Billing group with handler-plain "Payment received". Recorded limit (fails safe): auto-reply-marked remittances abstain at Rule 0. Registry-trusted: the 100000013 choiceset row (delta recorded applied; live taxonomy_version 3 + SPA plumbing corroborate).
