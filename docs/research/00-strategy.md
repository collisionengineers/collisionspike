...
| **3** | **Address-matching service: resolve a Case's part-postcode `Loc` → linked yard's full address → `InspectionAddress` → EVA field 9.** District match = `startswith(outwardCode)`; honour `AZURE_MAPS_ENABLED=false` → postcode.io. | **High** | **M** | The single largest *data-quality* gap in EVA submissions: **57% of located cases have only a district**, and EVA field 9 needs a full address (or an explicit "Image Based Assessment" decision — never silent). Without this, a human hand-resolves the majority of cases. | **Move #2** must land first (the matcher reads `Repairer` known-sites + `InspectionAddress` rows the corpus seeds). The address-policy *gate* is already built; this adds the *resolver*. Offline-buildable (postcode.io is free). |
...
**Move 3 (Claude, depends on Move 2) — Build the address-matching service.**
With the corpus seeded (Move 2) and intake feeding real Cases (Move 1), the resolver turns the **57%-part-postcode** problem from a per-case manual lookup into automatic resolution: district `startswith(outwardCode)` → linked yard → `InspectionAddress` → EVA field 9, postcode.io-normalised, with the built policy gate ensuring no silent "Image Based Assessment." It's offline-buildable and the clearest *measurable* win over the predecessor (which resolved no addresses at all).

**Why this order:** Move 1 is the only true blocker and it's not Claude's to do — hand it to the operator now, run concurrently. Move 2 precedes Move 3 by a hard data dependency. EVA-export validation (shortlist #4) is the natural fourth move — it needs operator EVA-test creds (B5) and benefits from Moves 2–3 producing a clean, address-resolved Case to export.
