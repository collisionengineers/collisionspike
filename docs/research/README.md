...
2. **Incorporate the confirmed provider corpus into Dataverse** — *Claude, in parallel*, pure data, no inbox contact (`plans/dataverse-corpus-incorporation.md`). Widens from a 45-row seed toward the real **active** base (the 137 off-jobsheet principals) and lays the `Repairer`/`InspectionAddress`/yard rows the next move needs.
