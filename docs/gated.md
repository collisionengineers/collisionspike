...
| **H2** | **Downstream flow-chain activation** (`classify-persist` / `parse` / `status-evaluate`) on digital@. | Re-arms the live webhook + rebinds Run-a-Child-Flow cards — designer work on the live flow. | Follow [./activation/m1-flow-chain-activation.md](./activation/m1-flow-chain-activation.md) in the designer. | 1d/2 |
