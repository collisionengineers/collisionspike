...
| **S12** | **EVA-validation (M2.B) deploy + publish + repoint.** | Infra deploy did **not** complete — the FC1 **plan-create** is **429-throttled** (persisted through 3 attempts + a 35-min watcher; an Azure-side subscription rate limit from the back-to-back evasentry/DI/evavalidation deploys, **not** a config issue — evasentry deployed fine on the identical bicep). FC1 is **one-app-per-plan** ([Learn](https://learn.microsoft.com/azure/azure-functions/flex-consumption-plan#considerations)), so it **cannot** reuse evasentry's plan; it needs its own new plan. **Finish when the throttle resets** (partial `cespkevalst…`/`-ai`/`-law` already exist → idempotent): `az deployment group create -g rg-collisionspike-dev -n evavalidation-deploy --template-file functions/evavalidation/infra/main.bicep` → `func azure functionapp publish <cespkeval-fn-…> --python` → repoint `status-evaluate` onto `cr1bd_evavalidation` (designer). The Function is built + verified (51 tests) + committed; nothing is lost. | Azure rate limit (time) + `[DEPLOY-WITH-LOGIN]`; designer repoint = live flow edit. |
