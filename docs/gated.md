...
| **H2** | **Downstream flow-chain activation** — **final designer step only** (API-safe prep DONE, PR #8). | classify-persist + status-evaluate re-imported + **ON**; storage `cespkevidstdev01` + `evidence` container, `cr1bd_evidenceblob` bound; `PDF_MAPPER_ENABLED=true`; `CS Intake` untouched. Remaining: in the designer, bind the parser connection on **CS Parse** → re-run `dataverse/.build/24-reimport-children.ps1` → turn CS Parse ON; then add 3 Run-a-Child cards to **CS Intake** (concurrency=1) + Save (re-arms the webhook). | Follow [./activation/m1-flow-chain-activation.md](./activation/m1-flow-chain-activation.md). | 1d/2 |
| **H3** | **Deploy the anchored exact-domain provider match** to the **live** intake — *before* seeding sender domains. | Editing the live intake flow is designer work; a substring collision binds the **wrong** provider → wrong Case/PO + Box prefix. | Operator redeploys the intake definition (the repo already carries `Filter_exact_domain`). Pairs with **S8**. | 2 |
