...
/**
 * Mint the next Case/PO for a (principal, year) under the advisory lock.
 *
 ...
 * @returns the formatted Case/PO, e.g. "CCPY26051".
 */
export async function mintCasePo(
  ...
  const seqRows = await q<{ next_seq: string | number }>(
    ...
    [`${prefix}%`, casePoSequenceRegex(p, yy), prefix],
  );
