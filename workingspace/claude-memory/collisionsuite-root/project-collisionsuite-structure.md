...
`active/spreadsheet-work/` = local copies of the active spreadsheets CE use (plain folder, not a repo, not tracked). The nested `collision-engineers-website` is the LIVE base44 site — see [[base44-website-push-guard]].

**2026-07-16 restructure:** `active/web-dev/` is now a **container folder** (like `connectors/`), NOT a repo: it holds `web-designs/` (own repo `collisionengineers/web-designs`, renamed from `web-dev` — design prototypes, canonical brand tokens `new-designs/_system/`, security review) + the live site at `current-website/collision-engineers-website/` which now has NO parent repo above it. Also: `active/collision-audaconnect/` = new repo (Audatex→AEG PDF prototype); `emailevals/` moved into `active/collisionspike/emailevals`; `archive/collisioncc` origin repointed to `collisionengineers/CollisionCC` (org copy is NEWER than the local archive snapshot); `archive/superseded-connectors/` never existed on disk (INDEX drift, now corrected). Suite map is machine-readable in `suite.manifest.json` — see [[suite-manifest-tooling]]; guidance policy: [[suite-agents-policy]].
