-- =============================================================================
-- 200_vehicle_data.sql -- immutable vehicle lookup + MOT estimator evidence
-- -----------------------------------------------------------------------------
-- The retained enrichment Function owns provider HTTP/auth and emits
-- vehicle-data.v1. TKT-151 may persist that contract here, but must not derive a
-- second estimator. All four evidence/result tables and the versioned model
-- profile are append-only: SELECT + INSERT only, with RLS policies in 900.
-- =============================================================================
BEGIN;
...
CREATE TABLE mileage_estimate_result (
  ...
  result_status            varchar(20) NOT NULL CHECK (
    result_status IN ('observed','estimated','range_only','insufficient')
  ),
  ...
  odometer_meaning         varchar(40) NOT NULL DEFAULT 'displayed_odometer'
                            CHECK (odometer_meaning = 'displayed_odometer'),
  target_date              date NOT NULL,
