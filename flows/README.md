...
| `parse.definition.json` | `Flow_Parse` | §5.5 | Reads **`PDF_MAPPER_ENABLED`** (value-or-default coalesce), branches: on → call parser connector **`ParseDocument`**, pre-fill the 13 EVA fields for **staff review** (never auto-final); off → audit skip, leave for manual entry. | **[RESERVED-FOR-USER]** |
