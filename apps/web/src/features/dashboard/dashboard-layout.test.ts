...
describe('dashboard simplification', () => {
  ...
  it('uses plain handler language', () => {
    const html = renderOverview();
    expect(html).not.toMatch(/Azure|Postgres|MSAL|Entra|JWT|Function|API|endpoint|webhook|Key Vault|OCR|JSON|operator-gated|deploy|mock|seed|schema|payload|provenance|\bBox\b/i);
  });
});
