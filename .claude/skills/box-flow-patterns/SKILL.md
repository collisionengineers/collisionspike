...
| 2 | `box-file-request-copy` — input `{caseId, fileRequestTemplateId, folderId}`; **guard `empty(folderId) OR empty(templateId)→folder_not_ready`** (never call Box with a null `folder.id` or an empty template path id); `CopyFileRequest status:"active"`; stamp `cr1bd_boxfilerequestid`+`url`; response `{fileRequestUrl, expiresAt, outcome}` (`outcome ∈ sent\|gated_off\|folder_not_ready`). **Standby** — the Code App calls the connector op DIRECTLY (no flow in the path); this flow is authored for future operator activation | `references/02-box-file-request-copy.md` | W2 / 04 §7 |
...
| 5 | `box-blob-purge` — `Recurrence` with `startTime`; `PurgeGraceDays` default 7; gate `BOX_API_ENABLED`; for cases at `box_synced AND boxsyncedat < now-grace`, delete the Blob bytes of **only the archived (accepted, non-excluded) IMAGE** evidence via `DeleteFile_V2`; clear `cr1bd_storagepath` only on confirmed delete (`Succeeded`) or already-gone (`Skipped`); **never the Box copy**, never the row, never non-image bytes (retained — a deferred follow-up) | `references/05-box-blob-purge.md` | W5 / 04 §11 |
