// identity-probe.mjs — prove the Box CCG service-identity grant and
// reach into the test folder. Run via Infisical so client_id/secret arrive as env:
//   infisical run --env dev -- node tools/box/identity-probe.mjs [enterpriseId]
...
if (Object.keys(cfg).some((key) => !allowedConfigKeys.has(key)) || cfg.mode !== 'test_only' || ALLOWED_ROOT !== TEST_ROOT) {
