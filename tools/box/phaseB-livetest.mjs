// phaseB-livetest.mjs — Phase B: prove the Box-native path live, scoped to the test
...
// ids are saved to .phaseB-state.json so `cleanup` removes everything (mandatory teardown).
//
//   1) node tools/box/phaseB-livetest.mjs setup --url https://<fn-host>/api/box-webhook?code=<key> [--template <fileRequestId>]
//   2) upload a file: via the printed File-Request URL (anonymous, the real unknown),
//      or:            node tools/box/phaseB-livetest.mjs upload-control   # authenticated control
//   3) watch the Function App Insights traces for the receiver order + FILE.UPLOADED
//   4) node tools/box/phaseB-livetest.mjs cleanup
//
...
const ROOT = String(JSON.parse(readFileSync(resolve(HERE, '..', 'box-scope.json'), 'utf8')).allowedRoot);
const STATE = resolve(HERE, '.phaseB-state.json');
...
if (cmd === 'setup') {
  ...
  writeFileSync(tmp, `phaseB control upload ${ROOT}\n`);
  ...
  console.log('usage: phaseB-livetest.mjs <setup --url <u> [--template <id>] | upload-control | cleanup>');
}
